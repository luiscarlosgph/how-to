...
2. Install the last release (you can find the last release version [here](https://github.com/facebookresearch/faiss/releases)):
   ...
   $ mkdir build
   $ cd build
   $ cmake -DFAISS_ENABLE_GPU=ON -DFAISS_ENABLE_PYTHON=ON -DBUILD_TESTING=OFF -DBUILD_SHARED_LIBS=ON -DFAISS_ENABLE_C_API=ON -DCMAKE_BUILD_TYPE=Release -DFAISS_OPT_LEVEL=avx2 -DBLA_VENDOR=Intel10_64_dyn -DMKL_LIBRARIES="-L/opt/intel/oneapi/mkl/latest/lib/intel64 -lmkl_intel_lp64 -lmkl_sequential -lmkl_core" -DCUDAToolkit_ROOT=/usr/local/cuda -DPython_EXECUTABLE=/root/.pyenv/shims/python -DPython_INCLUDE_DIRS=/root/.pyenv/versions/3.10.12/include -DPython_LIBRARIES=/root/.pyenv/versions/3.10.12/lib ..
   $ make -j faiss
   $ make -j swigfaiss
   $ sudo make install
